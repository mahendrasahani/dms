    ...
    <div class="page-titles">
        <div class="row">
            <div class="col-lg-8 col-md-6 col-12 align-self-center">
                <h1 class="mb-0 fw-bold">Hotels</h1>
            </div>
            <div class="col-lg-4 col-md-6 d-none d-md-flex align-items-center justify-content-end">
                <a href="{{route('backend.hotals.create')}}">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table id="zero_config" class="table table-striped table-bordered text-nowrap">
                                ...
                                <tbody>
                                    @php $count = '1'; @endphp
                                    @foreach ($hotal as $hotal)
                                    <tr>
                                        <td>{{$count++}}</td>
                                        <td>{{$hotal->name}}</td>
                                        <td>{{$hotal->location}}</td>
                                        <td>{{$hotal->user_id}}</td>
                                        <td>
                                            <div class="form-check form-switch">
                                                <input class="form-check-input" data-id="{{$hotal->id}}"
                                                    {{ $hotal->status === 1 ? 'checked' : '' }}
                                                    value="{{ $hotal->status }}" data-status="{{ $hotal->status }}"
                                                    type="checkbox" id="changeStatus" />
                                            </div>
                                        </td>
                                        <td>
                                            <div class="button-container">
                                                <a href="{{route('backend.hotals.create',[$hotal->id])}}">
