                ...
                <div class="card">
                    <div class="card-body">
                        <div class="table-responsive">
                                ...
                                <tbody>
                                    ...
                                    @foreach($team_leaders as $tl)
                                        ...
                                        <td>{{$sn++}}</td>
                                            <td>{{$tl->name ?? ''}}</td> 
                                            <td>{{$tl->email}}</td> 
                                            <td>{{$tl->getDepartmentType?->name ?? ''}}</td> 
                                            <td>{{$tl->getHead?->name ?? ''}}</td> 
                                            <td>{{$tl->getUnit?->name ?? ''}}</td>
                                            <td>{{$tl->getManager?->name ?? ''}}</td> 
                                            <td>
                                                <label class="switch">
                                                    <input type="checkbox" {{$tl->status == 1 ? "checked":""}} value="1" name="status" id="status" data-id="{{$tl->id}}" {{Auth::user()->role_type_id != 1 ? "disabled":""}}>
                                                    <span class="slider round"></span>
                                                </label>
                                            </td> 
                                            <td>{{Carbon\Carbon::parse($tl->created_at)->format('d M, Y')}}</td>
                                            <td>
                            ...
                            {{$team_leaders->links('pagination::bootstrap-5')}} 
                        </div>
                    </div>
                </div>
            </div>
